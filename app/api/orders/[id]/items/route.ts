...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await serializable(async (tx) => {
      ...
      if (![OrderStatus.OPEN, OrderStatus.IN_PROGRESS, OrderStatus.READY].some((status) => status === current.status)) throw new Error("NOT_EDITABLE");
      ...
      const kitchenTicketId = product.kitchenStationId
        ? await sendOrderItemToKitchen(tx, {
            orderId: current.id,
            orderNumber: current.number,
            orderItemId: item.id,
            kitchenStationId: product.kitchenStationId,
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityId: item.id,
          metadata: { orderId: current.id, productId: product.id, quantity: quantity.toString(), unitPrice: product.price.toString(), automaticKitchenDispatch: Boolean(kitchenTicketId) },
        },
      });
      return { order: updated, stockChanged, kitchenTicketId, orderItemId: item.id };
    });
    ...
    if (result.kitchenTicketId) publishEvent("kitchen.changed", { ticketId: result.kitchenTicketId, orderId: id });
    if (result.kitchenTicketId) void enqueueKitchenTicketPrint(result.kitchenTicketId, [result.orderItemId]).catch(() => undefined);
    ...
    if (message === "KITCHEN_LOCKED") return Response.json({ error: "La comanda ya está siendo preparada" }, { status: 409 });
