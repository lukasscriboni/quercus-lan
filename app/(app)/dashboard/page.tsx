...
export default function DashboardPage() {
  ...
  const metrics: Metric[] = [
    ...
    { label: "Ticket promedio", value: data ? money.format(Number(data.averageTicket)) : "—", icon: ClipboardCheck, accent: "text-[#ad9fe8]", note: "Por pedido", hideable: true },
    { label: "Mesas ocupadas", value: data?.tablesOccupied ?? "—", icon: Grid2X2, accent: "text-[#65b9e6]", note: "En este momento" },
  ];
  return (
    <div className="mx-auto max-w-[1500px]">
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div><p className="eyebrow mb-2">Sucursal principal</p><h1 className="text-3xl font-black tracking-[-.035em] sm:text-4xl">Pulso del local</h1><p className="mt-2 text-sm text-[#8f9d96]">Información compartida por todas las terminales de la red.</p></div>
      ...
      <section className="mt-5 grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
        <article className="surface p-5 sm:p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="surface-soft p-4"><p className="text-xs font-semibold text-[#899790]">Productos activos</p><p className="mt-2 text-3xl font-black">{data?.activeProducts ?? "—"}</p><p className="mt-2 text-xs text-[#65736d]">Listos para operar</p></div>
            <div className={`rounded-xl border p-4 ${(data?.criticalStock ?? 0) > 0 ? "border-danger/30 bg-danger/5" : "border-line bg-panel-soft"}`}><p className="flex items-center gap-2 text-xs font-semibold text-[#899790]"><AlertTriangle className={`h-4 w-4 ${(data?.criticalStock ?? 0) > 0 ? "text-danger" : "text-mint"}`} />Stock crítico</p><p className="mt-2 text-3xl font-black">{data?.criticalStock ?? "—"}</p><p className="mt-2 text-xs text-[#65736d]">En mínimo o por debajo</p></div>
          </div>
        </article>
        <article className="surface p-5 sm:p-6">
          <p className="eyebrow mb-2">Estado</p><h2 className="text-xl font-black">Sistema local</h2>
          <div className="mt-6 space-y-4 text-sm"><div className="flex items-center justify-between border-b border-line pb-3"><span className="text-[#8d9a94]">Aplicación</span><span className="pill border-mint/20 bg-mint/10 text-mint">Activa</span></div><div className="flex items-center justify-between border-b border-line pb-3"><span className="text-[#8d9a94]">Datos</span><span className="font-semibold">PostgreSQL local</span></div><div className="flex items-center justify-between"><span className="text-[#8d9a94]">Internet</span><span className="font-semibold text-mint">No requerido</span></div></div>
        </article>
      </section>
    </div>
  );
