...
const movementMeta: Record<CashMovement["type"], { label: string; className: string }> = {
  OPENING: { label: "Apertura", className: "text-[#a5a5a5]" },
  ...
  CLOSING: { label: "Cierre", className: "text-[#a5a5a5]" },
};
...
export default function CashPage() {
  ...
  const [virtualClosingAmount, setVirtualClosingAmount] = useState("0");
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div><p className="eyebrow mb-2">Turnos, cobros y efectivo</p><h1 className="text-3xl font-black tracking-[-.035em] sm:text-4xl">Caja</h1><p className="mt-2 text-sm text-[#999999]">Conectada con las cuentas del salón y guardada en el servidor local.</p></div>
      ...
      {!data?.openShift ? (
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_420px]">
          ...
          <form onSubmit={openShift} className="surface h-fit p-5 sm:p-6">
            ...
            <label className="mt-5 block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Caja</span><select className="field" required value={registerId} onChange={(event) => setRegisterId(event.target.value)}>{data?.registers.map((register) => <option key={register.id} value={register.id}>{register.name}</option>)}</select></label>
            <label className="mt-4 block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Efectivo inicial</span><input type="number" min="0" step="0.01" required className="field" value={openingAmount} onChange={(event) => setOpeningAmount(event.target.value)} /></label>
        ...
        <>
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-mint/25 bg-mint/5 p-4">
            <div className="flex items-center gap-3"><span className="live-dot h-3 w-3 rounded-full bg-mint" /><div><p className="font-black">{data.openShift.registerName} · turno abierto</p><p className="mt-0.5 text-xs text-[#888888]">Desde {dateTime.format(new Date(data.openShift.openedAt))} · {data.openShift.openedBy.displayName}</p></div></div>
            <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setShowMovement(true)} className="button-secondary"><CircleDollarSign className="h-4 w-4" />Nuevo movimiento</button><button type="button" onClick={() => { setClosingAmount("0"); setVirtualClosingAmount("0"); setShowClose(true); }} className="button-secondary text-amber"><LockKeyhole className="h-4 w-4" />Cerrar caja</button></div>
          </div>
          ...
          <div className="grid items-start gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,.65fr)]">
            <section className="surface overflow-hidden">
              ...
              <div className="p-3 sm:p-4">
                {requestedOrders.length === 0 ? <div className="py-12 text-center"><ReceiptText className="mx-auto h-8 w-8 text-[#575757]" /><p className="mt-3 text-sm font-bold">No hay mesas esperando el cobro</p><p className="mt-1 text-xs text-[#787878]">Cuando una mesa solicite la cuenta aparecerá automáticamente.</p></div> : <div className="space-y-2">{requestedOrders.map((order) => <OrderRow key={order.id} order={order} disabled={busy} pay={() => setCheckoutOrder(order)} />)}</div>}
              </div>
              {otherOrders.length > 0 && <div className="border-t border-line px-5 py-4"><p className="mb-3 text-[10px] font-bold uppercase tracking-[.15em] text-[#767676]">Otras cuentas abiertas</p><div className="flex flex-wrap gap-2">{otherOrders.map((order) => <span key={order.id} className="rounded-xl border border-line bg-ink px-3 py-2 text-xs text-[#909090]"><strong className="text-cream">{order.table?.name ?? `#${order.number}`}</strong> · {statusLabel[order.status]} · {money.format(Number(order.total))}</span>)}</div></div>}
            </section>
            ...
            <section className="surface overflow-hidden">
              ...
              <div className="max-h-[520px] overflow-y-auto p-3">{data.openShift.movements.length === 0 ? <p className="py-10 text-center text-sm text-[#7d7d7d]">Todavía no hay movimientos.</p> : data.openShift.movements.map((movement) => <div key={movement.id} className="flex items-center gap-3 border-b border-line/70 px-2 py-3 last:border-0"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-ink ${movementMeta[movement.type].className}`}>{movement.type === "EXPENSE" || movement.type === "WITHDRAWAL" ? <ArrowUpFromLine className="h-4 w-4" /> : <ArrowDownToLine className="h-4 w-4" />}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{movement.reason}</p><p className="mt-0.5 text-[10px] text-[#777777]">{movementMeta[movement.type].label} · {dateTime.format(new Date(movement.createdAt))} · {movement.user.displayName}</p></div><p className={`shrink-0 text-sm font-black ${movementMeta[movement.type].className}`}>{["EXPENSE", "WITHDRAWAL", "REFUND"].includes(movement.type) ? "−" : "+"}{money.format(Number(movement.amount))}</p></div>)}</div>
            </section>
          </div>
        </>
      )}

      {showMovement && data?.openShift && <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && !busy && setShowMovement(false)}><form onSubmit={addMovement} className="surface w-full max-w-md p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="eyebrow mb-1">Caja abierta</p><h2 className="text-2xl font-black">Nuevo movimiento</h2></div><button type="button" disabled={busy} onClick={() => setShowMovement(false)} className="rounded-xl border border-line p-2 text-[#929292]"><X className="h-4 w-4" /></button></div><label className="mt-5 block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Tipo</span><select className="field" value={movementType} onChange={(event) => setMovementType(event.target.value as typeof movementType)}><option value="INCOME">Ingreso de efectivo</option><option value="EXPENSE">Gasto</option><option value="WITHDRAWAL">Retiro de efectivo</option></select></label><label className="mt-4 block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Monto</span><input autoFocus type="number" min="0.01" step="0.01" required className="field" value={movementAmount} onChange={(event) => setMovementAmount(event.target.value)} /></label><label className="mt-4 block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Motivo</span><input required minLength={3} maxLength={200} className="field" value={movementReason} onChange={(event) => setMovementReason(event.target.value)} placeholder="Ej. Pago a proveedor" /></label><div className="mt-6 flex justify-end gap-2"><button type="button" disabled={busy} onClick={() => setShowMovement(false)} className="button-secondary">Cancelar</button><button disabled={busy} className="button-primary">{busy && <LoaderCircle className="h-4 w-4 animate-spin" />}Registrar</button></div></form></div>}
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && !busy && setShowClose(false)}>
          <form onSubmit={closeShift} className="surface max-h-[92vh] w-full max-w-lg overflow-y-auto p-5 sm:p-6">
            <div className="flex items-start justify-between">
              ...
              <button type="button" disabled={busy} onClick={() => setShowClose(false)} className="rounded-xl border border-line p-2 text-[#929292]"><X className="h-4 w-4" /></button>
            </div>

            <div className="mt-5 grid grid-cols-1 gap-2 sm:grid-cols-3">
              <div className="rounded-xl border border-line bg-ink p-3">
                <p className="text-[11px] font-semibold text-[#868686]">Ventas en efectivo</p>
                <p className="mt-1 text-lg font-black text-mint">{money.format(Number(data.openShift.cashSales))}</p>
              </div>
              <div className="rounded-xl border border-line bg-ink p-3">
                <p className="text-[11px] font-semibold text-[#868686]">Ventas virtuales</p>
              ...
              <div className="rounded-xl border border-amber/30 bg-amber/5 p-3">
                <p className="text-[11px] font-semibold text-[#8c8c8c]">Total vendido</p>
            ...
            {data.openShift.paymentBreakdown.length > 0 && (
              <div className="mt-3 rounded-xl border border-line bg-ink px-4 py-3">
                <p className="mb-2 text-[10px] font-bold uppercase tracking-[.14em] text-[#717171]">Detalle por medio de pago</p>
                <div className="space-y-2">
                  {data.openShift.paymentBreakdown.map((method) => (
                    <div key={method.id} className="flex items-center justify-between gap-3 text-sm">
                      <span className="text-[#aaaaaa]">{method.name} <span className="text-[10px] text-[#6d6d6d]">({method.count})</span></span>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <label className="rounded-xl border border-line bg-ink p-4">
                <span className="block text-xs font-semibold text-[#a4a4a4]">Efectivo declarado</span>
                <span className="mt-1 block text-[10px] text-[#727272]">Esperado: {money.format(Number(data.openShift.expectedCash))}</span>
                ...
                <span className={`mt-2 block text-xs font-black ${closingAmount === "" ? "text-[#6f6f6f]" : balanceClass(Number(closingAmount) - Number(data.openShift.expectedCash))}`}>{closingAmount === "" ? "Pendiente de declarar" : balanceLabel(Number(closingAmount) - Number(data.openShift.expectedCash))}</span>
              </label>
              <label className="rounded-xl border border-line bg-ink p-4">
                <span className="block text-xs font-semibold text-[#a4a4a4]">Dinero virtual declarado</span>
                <span className="mt-1 block text-[10px] text-[#727272]">Registrado: {money.format(Number(data.openShift.virtualSales))}</span>
                ...
                <span className={`mt-2 block text-xs font-black ${virtualClosingAmount === "" ? "text-[#6f6f6f]" : balanceClass(Number(virtualClosingAmount) - Number(data.openShift.virtualSales))}`}>{virtualClosingAmount === "" ? "Pendiente de declarar" : balanceLabel(Number(virtualClosingAmount) - Number(data.openShift.virtualSales))}</span>
              </label>
            </div>
            <div className="mt-3 flex items-center justify-between gap-4 rounded-xl border border-amber/25 bg-amber/5 px-4 py-3">
              <div><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#828282]">Resultado total del cierre</p><p className="mt-1 text-[11px] text-[#6f6f6f]">Efectivo más dinero virtual</p></div>
              {closingAmount !== "" && virtualClosingAmount !== "" ? <strong className={`text-base ${balanceClass((Number(closingAmount) - Number(data.openShift.expectedCash)) + (Number(virtualClosingAmount) - Number(data.openShift.virtualSales)))}`}>{balanceLabel((Number(closingAmount) - Number(data.openShift.expectedCash)) + (Number(virtualClosingAmount) - Number(data.openShift.virtualSales)))}</strong> : <strong className="text-sm text-[#777777]">Pendiente</strong>}
            </div>
...
function SummaryCard({ label, value, icon: Icon, highlight = false }: { label: string; value: string | number; icon: typeof Banknote; highlight?: boolean }) {
  return <div className={`surface p-4 ${highlight ? "border-amber/35 bg-amber/5" : ""}`}><div className="flex items-center justify-between"><p className="text-xs font-bold text-[#909090]">{label}</p><Icon className={`h-4 w-4 ${highlight ? "text-amber" : "text-mint"}`} /></div><p className={`mt-3 text-2xl font-black ${highlight ? "text-amber" : ""}`}>{money.format(Number(value))}</p></div>;
}

function OrderRow({ order, disabled, pay }: { order: PendingOrder; disabled: boolean; pay: () => void }) {
  return <div className="grid gap-3 rounded-2xl border border-amber/25 bg-amber/5 p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate text-lg font-black">{order.table?.name ?? `Pedido #${order.number}`}</h3><span className="rounded-full bg-amber/12 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-amber">Pide cuenta</span></div><p className="mt-1 text-xs text-[#878787]">{order.table?.sectorName ?? "Mostrador"} · Pedido #{order.number} · {order.guestCount} personas</p></div><button type="button" disabled={disabled} onClick={pay} className="button-primary min-w-40"><Banknote className="h-4 w-4" />Revisar cobro · {money.format(Number(order.total))}</button></div>;
}
...
function DirectSaleModal({ shift, methods, close, completed }: { shift: OpenShift; methods: PaymentMethod[]; close: () => void; completed: (sale: { number: number; paymentMethod: string }) => void | Promise<void> }) {
  ...
  useEffect(() => {
    if (searchMode === "barcode") { setProducts([]); setCatalogLoading(false); return; }
    const timer = window.setTimeout(() => loadCatalog(query, categoryId), 180);
    return () => window.clearTimeout(timer);
  }, [categoryId, loadCatalog, query, searchMode]);
  useEffect(() => {
    const refresh = (event: Event) => {
      const type = (event as CustomEvent<{ type?: string }>).detail?.type;
      if ((type === "products.changed" || type === "categories.changed") && searchMode === "name") loadCatalog(query, categoryId);
    };
    window.addEventListener("quercus:update", refresh);
    return () => window.removeEventListener("quercus:update", refresh);
  }, [categoryId, loadCatalog, query, searchMode]);
  ...
  async function handleProductSearchEnter() {
    ...
    if (searchMode === "barcode") {
      const matches = await loadCatalog(query, categoryId);
      if (matches.length === 1) {
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-3 backdrop-blur-sm sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && !busy && close()}>
      <div role="dialog" aria-modal="true" aria-labelledby="direct-sale-title" className="surface flex h-[min(850px,94vh)] w-full max-w-6xl flex-col overflow-hidden">
        <header className="flex items-center gap-3 border-b border-line p-4 sm:px-5">
          ...
          <button type="button" disabled={busy} onClick={close} className="rounded-xl border border-line p-2.5 text-[#929292] hover:text-cream"><X className="h-4 w-4" /></button>
        </header>
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_390px]">
          <section className="flex min-h-0 flex-col border-b border-line lg:border-b-0 lg:border-r">
            <div className="border-b border-line p-3 sm:p-4">
              ...
              {query.trim() && !catalogLoading && products.length === 1 && <p className="mt-2 text-[10px] font-bold uppercase tracking-[.12em] text-mint">Enter · agregar {products[0].name}</p>}
              <div className="mobile-scroll mt-2.5 flex gap-2 overflow-x-auto pb-1"><button type="button" onClick={() => setCategoryId("")} className={`shrink-0 rounded-xl border px-3 py-2 text-[11px] font-bold ${!categoryId ? "border-amber bg-amber/10 text-amber" : "border-line bg-ink text-[#929292]"}`}>Todos</button>{categories.map((category) => <button type="button" key={category.id} onClick={() => setCategoryId(category.id)} className={`shrink-0 rounded-xl border px-3 py-2 text-[11px] font-bold ${categoryId === category.id ? "border-amber bg-amber/10 text-amber" : "border-line bg-ink text-[#929292]"}`}>{category.name}</button>)}</div>
            </div>
            <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">{products.map((product) => <button type="button" key={product.id} disabled={busy} onClick={(event) => { addProduct(product); if (event.detail === 0) setQuery(""); }} className="group min-h-[112px] rounded-2xl border border-line bg-[#202020] p-3 text-left transition hover:border-amber/50 hover:bg-[#2b2b2b] active:scale-[.98] disabled:opacity-45"><p className="line-clamp-2 text-sm font-black leading-tight">{product.name}</p><p className="mt-1 truncate text-[9px] font-semibold uppercase tracking-wider text-[#6e6e6e]">{product.categoryName}</p><div className="mt-3 flex items-end justify-between gap-1"><span className="text-xs font-black text-amber">{money.format(Number(product.price))}</span><span className="grid h-7 w-7 place-items-center rounded-lg bg-amber/10 text-amber group-hover:bg-amber group-hover:text-ink"><Plus className="h-4 w-4" /></span></div></button>)}</div>{!catalogLoading && products.length === 0 && <div className="py-16 text-center text-sm text-[#7e7e7e]">No se encontraron productos.</div>}</div>
          </section>

          <aside className="flex min-h-0 flex-col bg-[#181818]">
            <div className="flex items-center justify-between border-b border-line px-4 py-3"><div><p className="text-sm font-black">Venta actual</p><p className="mt-0.5 text-[10px] text-[#787878]">{cartItems.reduce((sum, item) => sum + item.quantity, 0)} unidades</p></div><span className="pill bg-ink text-[#a4a4a4]">Sin mesa</span></div>
            <div className="min-h-0 flex-1 space-y-2 overflow-y-auto p-3">{cartItems.length === 0 ? <div className="grid h-full place-items-center text-center"><div><ShoppingCart className="mx-auto h-8 w-8 text-[#585858]" /><p className="mt-3 text-sm font-bold">Carrito vacío</p><p className="mt-1 text-xs text-[#787878]">Seleccioná productos del catálogo.</p></div></div> : cartItems.map((item) => <div key={item.product.id} className="rounded-xl border border-line bg-panel p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-bold">{item.product.name}</p><p className="mt-1 text-xs text-[#787878]">{money.format(Number(item.product.price))} c/u</p></div><p className="shrink-0 text-sm font-black text-amber">{money.format(Number(item.product.price) * item.quantity)}</p></div><div className="mt-2 flex items-center justify-between"><div className="flex items-center rounded-xl border border-line bg-ink"><button type="button" disabled={busy} onClick={() => changeQuantity(item.product.id, item.quantity - 1)} className="grid h-9 w-9 place-items-center text-[#989898] hover:text-cream"><Minus className="h-4 w-4" /></button><span className="min-w-8 text-center text-sm font-black">{item.quantity}</span><button type="button" disabled={busy || item.quantity >= 100} onClick={() => changeQuantity(item.product.id, item.quantity + 1)} className="grid h-9 w-9 place-items-center text-[#989898] hover:text-cream disabled:opacity-40"><Plus className="h-4 w-4" /></button></div><button type="button" disabled={busy} onClick={() => changeQuantity(item.product.id, 0)} title="Quitar producto" className="grid h-9 w-9 place-items-center rounded-lg text-[#888888] hover:bg-danger/10 hover:text-danger"><Trash2 className="h-4 w-4" /></button></div></div>)}</div>
            <div className="border-t border-line p-4"><div className="flex items-end justify-between"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#787878]">Subtotal</p><p className="text-2xl font-black text-amber">{money.format(total)}</p></div><button type="button" disabled={busy || cartItems.length === 0} onClick={() => setShowCheckout(true)} className="button-primary mt-4 w-full"><Banknote className="h-4 w-4" />Continuar al cobro <kbd className="rounded-md border border-black/20 bg-black/10 px-1.5 py-0.5 text-[9px]">Ctrl ⇧ C</kbd></button></div>
          </aside>
        </div>
      </div>
