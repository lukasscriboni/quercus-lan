  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <div className="surface max-h-[90vh] w-full max-w-4xl overflow-hidden">
        <div className="flex items-start justify-between border-b border-line p-5 sm:p-6"><div><p className="eyebrow mb-1">Trazabilidad</p><h2 className="text-2xl font-black">{product.name}</h2><p className="mt-1 text-xs text-[#858585]">{product.sku ?? "Sin SKU"} · últimos 100 movimientos</p></div><button onClick={close} className="rounded-xl border border-line p-2 text-[#929292]"><X className="h-4 w-4" /></button></div>
        <div className="max-h-[65vh] overflow-auto"><table className="w-full min-w-[720px] text-left"><thead className="sticky top-0 bg-[#141414] text-[10px] uppercase tracking-[.12em] text-[#7b7b7b]"><tr><th className="px-5 py-3">Fecha</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3 text-right">Cantidad</th><th className="px-4 py-3 text-right">Anterior → nuevo</th><th className="px-5 py-3">Responsable / detalle</th></tr></thead><tbody className="divide-y divide-line">{loading ? <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-[#7d7d7d]">Cargando historial…</td></tr> : rows.length === 0 ? <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-[#7d7d7d]">Todavía no hay movimientos.</td></tr> : rows.map((row) => <tr key={row.id}><td className="whitespace-nowrap px-5 py-4 text-xs text-[#949494]">{new Date(row.createdAt).toLocaleString("es-AR", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}</td><td className="px-4 py-4 text-sm font-bold">{movementLabels[row.type] ?? row.type}</td><td className={`px-4 py-4 text-right font-black ${Number(row.quantity) > 0 ? "text-mint" : "text-danger"}`}>{Number(row.quantity) > 0 ? "+" : ""}{Number(row.quantity).toLocaleString("es-AR")}</td><td className="px-4 py-4 text-right text-sm text-[#a9a9a9]">{Number(row.previousQty).toLocaleString("es-AR")} → {Number(row.newQty).toLocaleString("es-AR")}</td><td className="px-5 py-4"><p className="text-sm">{row.user?.displayName ?? "Sistema"}</p><p className="mt-1 text-xs text-[#7b7b7b]">{row.reason}</p></td></tr>)}</tbody></table></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && !busy && close()}>
      <form onSubmit={submit} className="surface max-h-[92vh] w-full max-w-lg overflow-y-auto p-5 sm:p-6">
        <div className="mb-6 flex items-start justify-between"><div><p className="eyebrow mb-1">Proveedores</p><h2 className="text-2xl font-black">Nuevo proveedor</h2><p className="mt-2 text-sm text-[#8d8d8d]">Solo el nombre es obligatorio.</p></div><button type="button" disabled={busy} onClick={close} className="rounded-xl border border-line p-2 text-[#929292] disabled:opacity-40"><X className="h-4 w-4" /></button></div>
        <div className="space-y-4">
          <label className="block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Nombre *</span><input required autoFocus maxLength={120} className="field" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre o razón social" /></label>
          <label className="block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Persona de contacto</span><input maxLength={120} className="field" value={contactName} onChange={(event) => setContactName(event.target.value)} placeholder="Opcional" /></label>
          <label className="block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">CUIT</span><input maxLength={30} className="field" value={taxId} onChange={(event) => setTaxId(event.target.value)} placeholder="Opcional" /></label>
          <label className="block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Teléfono</span><input type="tel" maxLength={40} className="field" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="Opcional" /></label>
          <label className="block"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Correo electrónico</span><input type="email" maxLength={160} className="field" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Opcional" /></label>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && !busy && close()}>
      <form onSubmit={submit} onKeyDown={(event) => { if (event.key === "Enter") event.preventDefault(); }} className="surface max-h-[92vh] w-full max-w-2xl overflow-y-auto p-5 sm:p-6">
        <div className="mb-6 flex items-start justify-between"><div><p className="eyebrow mb-1">Catálogo</p><h2 className="text-2xl font-black">Nuevo producto</h2><p className="mt-2 text-sm text-[#8d8d8d]">Solo el nombre es obligatorio. Se activará el control directo de stock.</p></div><button type="button" disabled={busy} onClick={close} className="rounded-xl border border-line p-2 text-[#929292] disabled:opacity-40"><X className="h-4 w-4" /></button></div>
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="sm:col-span-2"><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Nombre *</span><input required autoFocus maxLength={200} className="field" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre del producto" /></label>
          <label><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">SKU</span><input maxLength={80} className="field" value={sku} onChange={(event) => setSku(event.target.value)} placeholder="Opcional" /></label>
          <label><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Código de barras</span><input maxLength={80} className="field" value={barcode} onChange={(event) => setBarcode(event.target.value)} placeholder="Opcional" /></label>
          <label><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Categoría</span><select className="field" value={categoryId} onChange={(event) => setCategoryId(event.target.value)}><option value="">Sin categoría</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
          <label><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Unidad</span><select className="field" value={unit} onChange={(event) => setUnit(event.target.value)}><option value="UNIT">Unidad</option><option value="GRAM">Gramo</option><option value="KILOGRAM">Kilogramo</option><option value="MILLILITER">Mililitro</option><option value="LITER">Litro</option></select></label>
          <label><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Costo</span><input type="number" min="0" step="0.01" className="field" value={cost} onChange={(event) => setCost(event.target.value)} placeholder="0" /></label>
          <label><span className="mb-2 block text-xs font-semibold text-[#a4a4a4]">Precio de venta</span><input type="number" min="0" step="0.01" className="field" value={price} onChange={(event) => setPrice(event.target.value)} placeholder="0" /></label>
        </div>
        {error && <div className="mt-4 rounded-xl border border-danger/30 bg-danger/10 px-4 py-3 text-sm text-[#ffb4a5]">{error}</div>}
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-3 backdrop-blur-sm sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && close()}>
      <div role="dialog" aria-modal="true" aria-labelledby="receipt-history-title" className="surface flex h-[min(850px,94vh)] w-full max-w-7xl flex-col overflow-hidden">
        <header className="flex flex-wrap items-start gap-4 border-b border-line p-4 sm:px-6 sm:py-5">
          ...
          <div className="min-w-0 flex-1"><p className="eyebrow mb-1">Trazabilidad de stock</p><h2 id="receipt-history-title" className="text-2xl font-black">Ingresos de mercadería</h2><p className="mt-1 text-xs text-[#868686]">Buscá por fecha y seleccioná un ingreso para consultar todos sus datos.</p></div>
          <button type="button" onClick={close} className="rounded-xl border border-line p-2.5 text-[#929292] hover:text-cream"><X className="h-4 w-4" /></button>
        </header>

        <form onSubmit={(event) => { event.preventDefault(); loadHistory(from, to); }} className="flex flex-wrap items-end gap-3 border-b border-line bg-[#181818] p-4 sm:px-6">
          <label className="min-w-[170px] flex-1 sm:max-w-[230px]"><span className="mb-1.5 block text-xs font-semibold text-[#999999]">Desde</span><input required type="date" className="field py-2.5" value={from} onChange={(event) => setFrom(event.target.value)} /></label>
          <label className="min-w-[170px] flex-1 sm:max-w-[230px]"><span className="mb-1.5 block text-xs font-semibold text-[#999999]">Hasta</span><input required type="date" className="field py-2.5" value={to} onChange={(event) => setTo(event.target.value)} /></label>
          ...
          <p className="pb-2 text-xs font-semibold text-[#848484] sm:ml-auto">{loading ? "Buscando…" : `${rows.length.toLocaleString("es-AR")} ingresos encontrados`}</p>
        </form>
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[minmax(0,1fr)_370px]">
          <section className="min-h-0 overflow-auto lg:border-r lg:border-line">
            <table className="w-full min-w-[760px] text-left">
              <thead className="sticky top-0 z-10 bg-[#141414] text-[10px] font-bold uppercase tracking-[.12em] text-[#7b7b7b]"><tr><th className="px-5 py-3">Fecha</th><th className="px-4 py-3">Producto</th><th className="px-4 py-3">Proveedor</th><th className="px-4 py-3 text-right">Cantidad</th><th className="w-12 px-3 py-3" /></tr></thead>
              <tbody className="divide-y divide-line">{loading ? <tr><td colSpan={5} className="px-5 py-16 text-center text-sm text-[#7d7d7d]">Buscando ingresos…</td></tr> : rows.length === 0 ? <tr><td colSpan={5} className="px-5 py-16 text-center"><PackagePlus className="mx-auto h-8 w-8 text-[#575757]" /><p className="mt-3 text-sm font-bold">No hay ingresos en este período</p><p className="mt-1 text-xs text-[#787878]">Probá con un rango de fechas más amplio.</p></td></tr> : rows.map((row) => <tr key={row.id} tabIndex={0} onClick={() => setSelected(row)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(row); } }} className={`cursor-pointer transition hover:bg-[#262626] focus:outline-none ${selected?.id === row.id ? "bg-amber/5" : ""}`}><td className="whitespace-nowrap px-5 py-4 text-xs text-[#949494]">{receiptDateTime.format(new Date(row.createdAt))}</td><td className="px-4 py-4"><p className="max-w-[260px] truncate text-sm font-bold">{row.product.name}</p><p className="mt-1 text-[10px] text-[#787878]">SKU {row.product.sku ?? "—"}</p></td><td className="px-4 py-4 text-sm text-[#a9a9a9]">{row.supplier?.name ?? "Sin proveedor"}</td><td className="px-4 py-4 text-right text-base font-black text-mint">+{Number(row.quantity).toLocaleString("es-AR")}</td><td className="px-3 py-4"><ChevronRight className="h-4 w-4 text-[#6e6e6e]" /></td></tr>)}</tbody>
            </table>
          </section>

          <aside className="min-h-0 overflow-y-auto bg-[#181818] p-5">
            {!selected ? <div className="grid h-full min-h-64 place-items-center text-center"><div><History className="mx-auto h-8 w-8 text-[#585858]" /><p className="mt-3 text-sm font-black">Seleccioná un ingreso</p><p className="mx-auto mt-1 max-w-[230px] text-xs leading-relaxed text-[#787878]">Acá vas a ver el proveedor, producto, cantidades y responsable.</p></div></div> : <div><p className="eyebrow mb-1">Detalle del ingreso</p><h3 className="text-xl font-black">{selected.product.name}</h3><p className="mt-1 text-xs text-[#838383]">{receiptDateTime.format(new Date(selected.createdAt))}</p><div className="mt-5 rounded-2xl border border-mint/20 bg-mint/5 p-4"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-[#888888]">Cantidad ingresada</p><p className="mt-1 text-3xl font-black text-mint">+{Number(selected.quantity).toLocaleString("es-AR")} <span className="text-sm">{unitLabels[selected.product.unit] ?? selected.product.unit}</span></p><p className="mt-2 text-xs text-[#8c8c8c]">Stock: {Number(selected.previousQty).toLocaleString("es-AR")} → {Number(selected.newQty).toLocaleString("es-AR")}</p></div><div className="mt-5 space-y-4"><ReceiptDetail label="Proveedor" value={selected.supplier?.name ?? "Sin proveedor"} /><ReceiptDetail label="CUIT" value={selected.supplier?.taxId || "No informado"} /><ReceiptDetail label="Producto / SKU" value={`${selected.product.name} · ${selected.product.sku ?? "Sin SKU"}`} /><ReceiptDetail label="Código de barras" value={selected.product.barcode || "No informado"} /><ReceiptDetail label="Responsable" value={selected.user ? `${selected.user.displayName} · ${selected.user.username}` : "Sistema"} /><ReceiptDetail label="Ubicación" value={selected.warehouse.name} /><ReceiptDetail label="Motivo registrado" value={selected.reason} /></div></div>}
          </aside>
        </div>
      </div>
    </div>
...
function ReceiptDetail({ label, value }: { label: string; value: string }) {
  return <div className="border-b border-line pb-3 last:border-0"><p className="text-[10px] font-bold uppercase tracking-[.12em] text-[#727272]">{label}</p><p className="mt-1 text-sm font-semibold leading-relaxed text-[#e0e0e0]">{value}</p></div>;
}
...
export default function StockPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/stock?warehouseId=${warehouseId}&q=${encodeURIComponent(query)}&searchMode=${searchMode}`, { cache: "no-store" });
      const payload = await response.json();
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow mb-2">Inventario</p><h1 className="text-3xl font-black tracking-[-.035em] sm:text-4xl">Stock</h1><p className="mt-2 text-sm text-[#999999]">Existencias del local e historial completo de movimientos.</p></div><div className="flex flex-wrap items-center gap-3"><button type="button" onClick={() => setShowReceiptHistory(true)} className="button-secondary"><CalendarDays className="h-4 w-4" />Historial de ingresos</button>{canAdjust && <button type="button" disabled={receiptLoading} onClick={() => setShowReceipt(true)} className="button-primary">{receiptLoading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <PackagePlus className="h-4 w-4" />}Ingresar mercadería</button>}<div className="surface-soft flex items-center gap-3 px-4 py-3"><TriangleAlert className={`h-5 w-5 ${critical ? "text-danger" : "text-mint"}`} /><div><p className="text-[10px] uppercase tracking-wider text-[#818181]">Stock crítico</p><p className="text-lg font-black">{critical}</p></div></div></div></div>
      <div className="max-w-xl"><ProductSearchModeControl mode={searchMode} /></div>
      <form onSubmit={(event) => { event.preventDefault(); load(); }} className="relative mb-4 w-full max-w-xl"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[#717171]" /><input className="field pl-10" value={query} onChange={(event) => setQuery(event.target.value)} aria-label={searchMode === "barcode" ? "Buscar stock por código de barras" : "Buscar stock por nombre"} placeholder={searchMode === "barcode" ? "Código de barras exacto…" : "Nombre del producto por partes…"} /></form>
      ...
      <div className="table-wrap overflow-x-auto"><table className="w-full min-w-[680px] text-left"><thead className="border-b border-line bg-[#141414] text-[10px] font-bold uppercase tracking-[.14em] text-[#797979]"><tr><th className="px-5 py-4">Producto</th><th className="px-4 py-4 text-right">Actual</th><th className="px-4 py-4 text-right">Mínimo</th><th className="px-5 py-4">Estado</th></tr></thead><tbody className="divide-y divide-line">{loading ? <tr><td colSpan={4} className="px-5 py-16 text-center text-sm text-[#7d7d7d]">Actualizando existencias…</td></tr> : products.map((product) => { const row = product.stocks[0]; const quantity = Number(row?.quantity ?? 0); const minimum = Number(row?.minimum ?? 0); const isCritical = quantity <= minimum; return <tr key={product.id} tabIndex={0} onClick={() => setHistory(product)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setHistory(product); } }} className="cursor-pointer transition hover:bg-[#262626] focus:bg-[#262626] focus:outline-none" title="Ver historial"><td className="px-5 py-4"><div className="flex items-start gap-3"><History className="mt-1 h-4 w-4 shrink-0 text-[#6f6f6f]" /><div><p className="font-bold">{product.name}</p><p className="mt-1 text-xs text-[#777777]">{product.sku ?? "Sin SKU"} · {unitLabels[product.unit] ?? product.unit}</p></div></div></td><td className="px-4 py-4 text-right text-lg font-black">{quantity.toLocaleString("es-AR")}</td><td className="px-4 py-4 text-right text-sm text-[#9b9b9b]">{minimum.toLocaleString("es-AR")}</td><td className="px-5 py-4"><span className={`pill ${isCritical ? "border-danger/25 bg-danger/10 text-danger" : "border-mint/20 bg-mint/10 text-mint"}`}>{isCritical ? "Crítico" : "Correcto"}</span></td></tr>; })}</tbody></table></div>
      {products.length === 0 && !loading && <div className="surface mt-4 p-10 text-center text-sm text-[#7f7f7f]">No hay artículos con control de stock para mostrar.</div>}
